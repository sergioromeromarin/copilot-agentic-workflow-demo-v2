...
export function canWritePath(policy: ToolPolicy, relativePath: string): boolean {
...
export async function readText(repoRoot: string, relativePath: string): Promise<string> {
  const abs = path.join(repoRoot, relativePath);
  return await fs.readFile(abs, 'utf8');
}

export async function writeText(policy: ToolPolicy, repoRoot: string, relativePath: string, content: string): Promise<void> {
